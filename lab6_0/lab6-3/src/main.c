...
void playSound(int freq){//freq is 10 times the actual frequency
	if(freq<=0){
		stopSound();
	}

	int period = 100000/freq;// period in # of count-ups

	SET_REG(RCC->APB1ENR1,1,1);
	TIM2->ARR = (uint32_t)period;
	TIM2->CCR2 = (uint32_t)period/2;
	TIM2->EGR = 1;
...
void stopSound(){
	...
	// disable TIM2
	SET_REG(TIM2->CR1,1,0);
	SET_REG(RCC->APB1ENR1,1,0);
...
int main(){
	...
	while(1){
		...
		if(value!=new_value){
			value = new_value;
			show_value(value);
			if((pressCol != -1) && (pressRow != -1)){
				playSound(freq[pressRow][pressCol]);
			}
			else{
				stopSound();
			}
